...
<template>
  ...
  <section v-else class="flex flex-col justify-center items-center h-screen w-screen gap-4">
    <h1 class="text-3xl">
      Who's that Pokémon? <span class="text-gray-500 text-sm">#{{ randomPokemon.id }}</span>
    </h1>
    <div class="h-5">
      <button
        v-if="gameStatus !== GameStatus.PLAYING"
        class="fade-in transition-all bg-black hover:bg-gray-800 text-white cursor-pointer py-1 px-2 rounded shadow-lg"
        @click="getNextRound()"
      >
        New Game
      </button>
